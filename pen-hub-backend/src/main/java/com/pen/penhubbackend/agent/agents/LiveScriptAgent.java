...
public class LiveScriptAgent implements NodeAction {
    ...
    public Map<String, Object> apply(OverAllState state) throws Exception {
        ...
        log.info("LiveScriptAgent 开始执行: topic={}", topic);

        String segmentsOutline = GsonUtils.toJson(outline.getSegments());
        String prompt = PromptConstant.LIVE_SCRIPT_PROMPT
                .replace("{topic}", topic)
        ...
        LiveScriptState.LiveScriptResult scriptResult = GsonUtils.fromJson(content, LiveScriptState.LiveScriptResult.class);

        log.info("LiveScriptAgent 执行完成: 环节数={}", scriptResult.getSegments() != null ? scriptResult.getSegments().size() : 0);
                ...
                .doOnNext(response -> {
                    ...
                    if (chunk != null && !chunk.isEmpty()) {
                        ...
                        if (streamHandler != null) {
                            streamHandler.accept(SseMessageTypeEnum.LIVE_SCRIPT_STREAMING.getStreamingPrefix() + chunk);
                        }
                    }
                })
                .doOnError(error -> log.error("LiveScriptAgent 流式调用失败", error))
                .blockLast();
