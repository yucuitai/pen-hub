...
import com.pen.penhubbackend.agent.parallel.ParallelImageGenerator;
                ...
                List<ArticleState.TitleOption> titleOptions = finalState.value(KEY_TITLE_OPTIONS)
                        .map(v -> {
                            if (v instanceof List) {
                                // 检查列表元素类型，如果是 TitleOption 则直接转换
                                List<?> list = (List<?>) v;
                                if (!list.isEmpty() && list.get(0) instanceof ArticleState.TitleOption) {
                                    @SuppressWarnings("unchecked")
                                    List<ArticleState.TitleOption> typed = (List<ArticleState.TitleOption>) v;
                                    return typed;
                                }
                            }
                            // 图框架可能将对象序列化为 Map，需要通过 JSON 中转反序列化
            ...
            if (result.isPresent()) {
                ...
                ArticleState.OutlineResult outline = finalState.value(KEY_OUTLINE)
                        .map(v -> {
                            ...
                            return GsonUtils.fromJson(
                                    GsonUtils.toJson(v),
                                    new com.google.gson.reflect.TypeToken<ArticleState.OutlineResult>(){}
                            );
                        })
                        .orElse(null);
            ...
            if (result.isPresent()) {
                ...
                List<ArticleState.ImageRequirement> imageRequirements = finalState.value(KEY_IMAGE_REQUIREMENTS)
                        .map(v -> {
                            if (v instanceof List) {
                                List<?> list = (List<?>) v;
                                if (!list.isEmpty() && list.get(0) instanceof ArticleState.ImageRequirement) {
                                    @SuppressWarnings("unchecked")
                                    List<ArticleState.ImageRequirement> typed = (List<ArticleState.ImageRequirement>) v;
                                    return typed;
                                }
                            }
                            return GsonUtils.fromJson(
                ...
                List<ArticleState.ImageResult> images = finalState.value(KEY_IMAGES)
                        .map(v -> {
                            if (v instanceof List) {
                                List<?> list = (List<?>) v;
                                if (!list.isEmpty() && list.get(0) instanceof ArticleState.ImageResult) {
                                    @SuppressWarnings("unchecked")
                                    List<ArticleState.ImageResult> typed = (List<ArticleState.ImageResult>) v;
                                    return typed;
                                }
                            }
                            return GsonUtils.fromJson(
